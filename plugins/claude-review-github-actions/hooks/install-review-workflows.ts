/**
 * GitHub Actions review workflow installation hook
 *
 * SessionStart hook that copies bundled review workflow files from the plugin directory
 * to the project's .github/workflows/ directory. Preserves existing workflows by
 * only copying files that don't already exist.
 *
 * This enables projects to automatically receive CI/CD review workflows for:
 * - Task validation against rules
 * - Documentation completeness review
 * - Commit message and structure review
 *
 * @module install-review-workflows
 */
...
/**
 * SessionStart hook handler for installing GitHub Actions review workflows
 *
 * Copies workflow files from the plugin's workflows/ directory
...
async function handler(input: SessionStartInput): Promise<SessionStartHookOutput> {
  const logger = createDebugLogger(input.cwd, 'install-review-workflows', true);
  ...
  try {
    ...
    const workflowsSourceDir = path.join(pluginRoot, 'workflows');
    ...
    for (const file of ymlFiles) {
      ...
      if (DEBUG) {
        if (result.copied) {
          console.log(`[install-review-workflows] ✓ Installed: ${file}`);
        } else if (result.existed) {
          console.log(`[install-review-workflows] - Already exists: ${file}`);
        } else if (result.error) {
          console.log(`[install-review-workflows] ✗ Error with ${file}: ${result.error}`);
        }
      }
    }
    ...
    if (copiedCount > 0) {
      messages.push(`✓ Installed ${copiedCount} review workflow(s):`);
      results
        .filter(r => r.copied)
    ...
    return {
      hookSpecificOutput: {
        hookEventName: 'SessionStart',
        additionalContext: `Review workflow installation error: ${error}`,
      },
    };
  }
}
